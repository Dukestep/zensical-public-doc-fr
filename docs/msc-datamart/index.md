...
!!! info inline end "Période de rétention"
    Une période de rétention de 30 jours est en vigueur pour tous les jeux de données.
...
La documentation de ce service se trouve sur la [page consacrée au protocole AMQP](amqp.md).
  
:material-information-outline: Note:  Les utilisateurs qui ont besoin de récupérer des données une seule fois peuvent utiliser [wget pour télécharger des données du Datamart du SMC](readme_wget-datamart.md). Veuillez vous référer à la [politique d'utilisation des services de données ouvertes](../usage-policy/index.md).
...
L'accès aux données via [le protocole AMQP est évidemment fortement conseillé](amqp.md).
...
* Tutoriels :
    * Utiliser les données dans le logiciel de bureau QGIS :
        * [Ajouter des données brutes](../usage/tutorial_raw-data_QGIS.md)
* [Licence d'utilisation finale](../licence/index.md)
* [Politique d'utilisation des services](../usage-policy/index.md)
