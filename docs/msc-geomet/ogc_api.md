...
- [x] Correspondances exactes : `=` recherche les entités pour lesquelles un attribut possède une valeur exacte.</br>
- [x] Chaînes partielles ou mots-clés : `LIKE` avec le caractère générique `%`.</br>
- [x] Opérateurs de comparaison : `>`, `<`, `>=`, `<=` permettent de filtrer les valeurs numériques ou temporelles.</br>
- [x] Opérateurs logiques : `AND`, `OR`, `NOT` permettent de comparer des conditions.</br>
- [x] Prédicats spatiaux : `INTERCEPTS`, `DISJOINT` permettent de filtrer selon la géométrie.</br>
